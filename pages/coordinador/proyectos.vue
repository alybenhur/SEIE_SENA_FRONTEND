  ...
  <NuxtLayout name="coordinador">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
      <div>
        ...
        <button disabled
          class="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-gray-500 px-4 py-2 rounded-lg bg-gray-200 border border-gray-300 cursor-not-allowed"
...
// PDF de la rúbrica de evaluación (compartido con la vista del investigador)
...
async function confirmarAsignacion(id: string) {
  ...
  procesando.value = true
  try {
    const actualizado = await patch<any>(`/proyectos/${id}/asignar-evaluador`, { evaluadorId: evaluadorSeleccionado.value })
    const idx = proyectos.value.findIndex(p => p._id === id)
    if (idx !== -1) {
      // Respeta el estado real devuelto (sustentando se conserva; aceptado pasa a evaluador_asignado)
      proyectos.value[idx].estado = actualizado?.estado ?? proyectos.value[idx].estado
